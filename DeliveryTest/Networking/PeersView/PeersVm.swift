...
extension PeersVm: PeersControllerDelegate {
    public func received(data: Data, viaStream: Bool) -> Bool {
        if let sendable = try? JSONDecoder().decode(PeerInfo.self, from: data) {
            ...
            setPeersList()
            return true
        }else  if let sendable = try? JSONDecoder().decode(SendableBall.self, from: data) {
          //  print("success decode in received as sendableball")
            //print("sender: \(sendable.sender)")
           // print("ball position \(sendable.transform.columns.3)")
        }else{
            print("failed Decode in peersVM")
        }
        return false
    }
    ...
    private func setPeersList() {
        ...
        let peerListStatic = peerList
        Task {
            await MainActor.run {
                self.peersList = peerListStatic
            }
        }
    }
}
