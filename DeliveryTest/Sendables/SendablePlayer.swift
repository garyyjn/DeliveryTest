...
struct SendableBall:PSSendable{
    var sender: String
    var timeSince1970: Double
    let state: String
    let transform: simd_float4x4
    init(state: String, playerTransform: simd_float4x4) {
        self.sender = PeersVm.shared.playerId
        self.state = state
...
struct SendablePlayer: PSSendable {
    ...
    init(state: fireballstate, playerTransform: simd_float4x4) {
        self.sender = PeersVm.shared.playerId
    ...
    var fireballState: fireballstate {
        fireballstate(rawValue: state) ?? .nonexistent
    }
}**/
